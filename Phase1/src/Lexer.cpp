...
void Lexer::next(Token& token) {
	...
	if (charinfo::isLetter(*BufferPtr)) {   // looking for keywords or identifiers like "int", a123 , ...
		...
		else if (Context == "else if") {                    //ch
			kind = Token::KW_elseif;
		}
		else if (Context == "else") {                      //ch
			kind = Token::KW_else;
		}
		else if (Context == "for") {                       //ch
			kind = Token::KW_for;
		}
        else if (Context == "while") {                     //ch
			kind = Token::KW_while;
		}
        else if (Context == "print") {                     //ch
			kind = Token::KW_print;
		}
		else if (Context == "and") {
			kind = Token::KW_and;
		...
		if (*BufferPtr == '=' && *(BufferPtr + 1) == '=') {      // ==
			...
			formToken(token, BufferPtr + 2, Token::start_comment);
		}
        else if (*BufferPtr == '*' && *(BufferPtr + 1) == '/') {      // */
			formToken(token, BufferPtr + 2, Token::end_comment);
		}
		else if (*BufferPtr == '=') {
...
#define CASE(ch, tok) \
            case ch: formToken(token, BufferPtr + 1, tok); break
				...
				CASE('%', Token::mod);
                CASE('{', Token::KW_begin);         // ch
                CASE('}', Token::KW_end);           // ch
